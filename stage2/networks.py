...
def define_S(input_nc, output_nc, ngf, netS, norm='batch', use_dropout=False, init_type='normal',
             init_gain=0.02, no_antialias=False, no_antialias_up=False, gpu_ids=[], opt=None, drop_out=True):
    net = None
    ...
    if netS == 'resnet_9blocks':
        ...
        net = UnetGenerator_SEG(input_nc, output_nc, ngf, bilinear=True, use_duse=True,
                                drop_out=drop_out).cuda()  # UNet with Spatial-Channel SE
    elif netS == 'r2unet':
        ...
        net = ADDUnetGenerator_SEG(input_nc, output_nc, ngf, bilinear=True,
                                   use_duse=True).cuda()  # Attention Dense U-Net with scSE
    else:
...
class UnetGenerator_SEG(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, bilinear=False, use_duse=False, drop_out=True):
...
class ADDUnetGenerator_SEG(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, bilinear=False, use_duse=False):
        ...
        self.Up5 = up_conv(ch_in=16 * ngf, ch_out=8 * ngf)
        self.Att5 = Attention_block(F_g=8 * ngf, F_l=8 * ngf, F_int=4 * ngf)
        self.Up_conv5 = dense_conv_block(ch_in=16 * ngf, ch_out=8 * ngf, nDenselayer=5, growthRate=32,
                                         use_duse=use_duse)
