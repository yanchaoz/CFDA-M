...
class CUT2SEGModel_TRAIN(BaseModel):
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'D_real', 'D_fake', 'G', 'NCE', 'CC', 'MIND', 'DICE', 'SC']
        ...
        if self.isTrain:
            ...
            self.criterionNCE = []
            self.criterionMIND = MINDLoss(non_local_region_size=9, patch_size=7, neighbor_size=3,
                                          gaussian_patch_sigma=3.0).cuda()
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_DICE > 0.0:
            ...
            self.loss_DICE = 0.0
        self.loss_SC = dice_loss_norm(self.seg_idt_B, self.idt_B_uc_seg) * self.opt.lambda_DICE

        self.loss_G = self.loss_G_GAN + loss_NCE_both + self.loss_MIND + self.loss_CC + self.loss_DICE + self.loss_SC
