...
def define_S(input_nc, output_nc, ngf, netS, norm='batch', use_dropout=False, init_type='normal',
    ...
    if netS == 'resnet_9blocks':
        ...
        net = UnetGenerator_SEG(input_nc, output_nc, ngf, bilinear=True,
                                use_duse=True).cuda()  # UNet with Spatial-Channel SE
    elif netS == 'r2unet':
        ...
        net = RSUNet([18, 32, 48, 64, 80], dp=0.5).cuda()
    elif netS == 'unet':
        net = UNet(1, 1).cuda()
    else:
...
class RSUNet(nn.Module):
    ...
    def __init__(self, nfeatures, dp, in_channels=1, depth=None, residual=True, upsample='bilinear'):
        ...
        self.embed_out = OutputMod(in_channels, 1, (3, 3))
